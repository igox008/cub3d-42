...
/*   Updated: 2024/08/18 15:48:29 by alaassir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	check_wall(int x, int y, t_game *game)
{
	...
	if (game->map[y][x] == '1' || game->map[y][x] == 'D' \
		|| game->map[tmp_y][tmp_x] == '1' || game->map[tmp_y][tmp_x] == 'D' \
		|| game->map[tmp_y][x] == '1' || game->map[tmp_y][x] == 'D' \
		|| game->map[y][tmp_x] == '1' || game->map[y][tmp_x] == 'D'\
		|| game->map[tmp_y_1][tmp_x] == '1' || game->map[tmp_y_1][tmp_x] == 'D' \
		|| game->map[tmp_y][tmp_x_1] == '1' || game->map[tmp_y][tmp_x_1] == 'D')
		return (1);
	return (0);
}
